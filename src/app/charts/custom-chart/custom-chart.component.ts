...
export class CustomChartComponent implements OnInit {
  ...
  ngOnInit() {
    this.GetVerticalBarCharData();
    this.GetChartValues();
  }


  GetChartValues() {
    let calculatedWidth = ((100 - this.ChartData.length + 1) / this.ChartData.length);
    if (calculatedWidth > 6) {
      calculatedWidth = 6;
    } else {
      calculatedWidth = parseInt(calculatedWidth);
    }
    ...
    for (let i = 0; this.ChartData.length > i; i++) {
      ...
      if ( i === 0 ) {
        ...
        this.ChartData[i].MarginLeft = ((i + 1) + (i * calculatedWidth)) + '%';
      }
